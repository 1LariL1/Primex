<?php
    ...
    $id_orders = $_GET['id'];
    $orders = mysqli_query($connect, query:"SELECT `orders`.`id_orders`, `orders`.`id_user`, `orders`.`id_service`, `orders`.`price`, `orders`.`status`, `orders`.`date`, `orders`.`time`, `orders`.`address`, `user`.`login`, `service`.`service` FROM `orders` JOIN `user` on `orders`.`id_user`=`user`.`id_user` join `service` on `orders`.`id_service`=`service`.`id_service` WHERE `id_orders` = '$id_orders'");
                ...
                <div class="admin-box">
                    ...
                    <div class="admin-wrap">
                        ...
                        <select name="ordersStatus" class="admin-input" required style="border: none; padding-right: 70px;">
                            ...
                            <option value="Готов к выполнению">Готов к выполнению</option>
